...
<triggers>
  ...
  <send>
    ...
    if scan == "%1" or string.gsub("%1", "s$", "") == scan then
    -- first we collect the name of the object
    -- After collecting the names, parse the coordinates
    starmap  = {}
...
return 0
    ...
    -- Handle the the sorting of starmap.
...
<aliases>
  ...
  <alias
   enabled="y"
   match="^sm([aAbcCdeEfijlLmMopPrstTuwxyY]|\.help)(\s\w+)?$"
   regexp="y"
   send_to="12"
   sequence="100"
  >
